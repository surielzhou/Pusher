...
- [ ] 测试完整链路：创建文章、生成草稿、编辑、提交 review、通过、发布准备、标记已发布。
- [ ] 测试退回链路：创建文章、生成、提交 review、退回、编辑、再次提交 review。
- [ ] 测试暂不发布链路：提交 review 后标记暂不发布，发布准备被阻断。
- [ ] 测试内容修改后重新 review：通过后修改正文，再进入发布准备被阻断。
- [ ] 运行 `npm test -- tests/integration/article-lifecycle.test.ts`，预期通过。
...
- [ ] E2E 覆盖新建文章。
- [ ] E2E 覆盖选择科技互联网、金融、文学任一方向生成草稿。
- [ ] E2E 覆盖编辑标题、摘要、正文和配图建议。
- [ ] E2E 覆盖提交 review。
- [ ] E2E 覆盖 review 通过。
- [ ] E2E 覆盖发布准备和复制内容。
- [ ] 运行 `npm run test:e2e -- tests/e2e/mvp-flow.spec.ts`，预期通过。
...
- [ ] Review 通过后能生成公众号可复制内容。
- [ ] 工作台能展示待编辑、待Review、待发布文章。
- [ ] 历史文章能按方向和状态筛选。
- [ ] 生成失败、发布失败、非法状态流转都有记录。
